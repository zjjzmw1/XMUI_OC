...
+ (UIButton *)getButtonImageTitleWithImage:(nullable UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor spacing:(float)spacing alignmentType:(ButtonImageTitleType)type aFont:(UIFont *)aFont {
    ...
    if (image) {
        ...
        // 整体宽度加宽。否则 sizeToFit之后就展示不全了
        [button setContentEdgeInsets:UIEdgeInsetsMake(0, 0, 0, spacing)];
        // 系统默认是 【图在左、文字在右。 没有间隙的】
        if (type == 0) { // 图片左 整体居中
            button.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
            [button setTitleEdgeInsets:UIEdgeInsetsMake(0, spacing, 0, -spacing)];
