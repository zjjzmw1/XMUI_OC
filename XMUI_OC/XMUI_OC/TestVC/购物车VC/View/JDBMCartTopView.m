...
@implementation JDBMCartTopView

/**
 获取自定义导航栏
 */
+ (instancetype)getInstanceWithTitle:(nullable NSString *)titleStr {
    JDBMCartTopView *naviV = [[JDBMCartTopView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, kNaviStatusBarH_XM)];
...
- (void)backAction {
    ...
    [[JDBMCartTopView getCurrentVC].navigationController popViewControllerAnimated:YES];
    }
}
...
+ (nullable UIViewController *)getCurrentVC {
    ...
    UIViewController *currentVC = [JDBMCartTopView getCurrentVCFrom:rootViewController];
    return currentVC;
}
...
+ (UIViewController *)getCurrentVCFrom:(UIViewController *)rootVC {
    ...
    if ([rootVC isKindOfClass:[UITabBarController class]]) {
        ...
        currentVC = [JDBMCartTopView getCurrentVCFrom:[(UITabBarController *)rootVC selectedViewController]];
    } else if ([rootVC isKindOfClass:[UINavigationController class]]) {
        // 根视图为UINavigationController
        currentVC = [JDBMCartTopView getCurrentVCFrom:[(UINavigationController *)rootVC visibleViewController]];
    } else {
        // 根视图为非导航类
        currentVC = rootVC;
    }
